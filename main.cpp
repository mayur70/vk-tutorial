...
SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice vkDevice)
{
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(vkDevice, surface,
        &details.capabilities);
    uint32_t formatCount;
    vkGetPhysicalDeviceSurfaceFormatsKHR(vkDevice, surface, &formatCount,
        nullptr);
    if (formatCount != 0) {
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(vkDevice, surface,
        &presentaionModeCount, nullptr);
    if (presentaionModeCount != 0) {
        details.presentModes.resize(presentaionModeCount);
        vkGetPhysicalDeviceSurfacePresentModesKHR(
            vkDevice, surface, &presentaionModeCount, details.presentModes.data());
    }
    return details;
}
...
void createGraphicsPipeline()
{
    ...
    dynamicState.dynamicStateCount = static_cast<uint32_t>(dynamicStages.size());
